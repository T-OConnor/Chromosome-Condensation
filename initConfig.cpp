...
void initDynamicsBrownianSimulation(vector<Chromosome> & vecChromosome, vector<Node> & vecNode, vector<int> & vecFluoroSite, string initType, vector<double> & randNum01, int argc, char ** argv)
{
    ...
    if (initType == "randomEllipsoid")
    {
        ...
        std::mt19937_64 generator(rd()); // Mersenne Twister engine
        std::normal_distribution<double> normdistw(0.0,width/2);
        std::normal_distribution<double> normdistL(0.0,length/2);
        ...
        //create random x,y,z arrays
        for (vector<Node>::iterator it = vecNode.begin(); it != vecNode.end(); it ++)
        {
            int index = std::distance(vecNode.begin(), it );


            x[index] = normdistw(generator);
            y[index] = normdistw(generator);
            z[index] = normdistL(generator);


        }

        //sort z array in ascending order
        //std::sort(std::begin(z), std::end(z));
        // std::sort(std::begin(z), std::end(z), std::less<double>{});
        std::sort(z, z+n);
    ...
    else if (initType == "randomCylinder")
    {
        double length = 125;
        double width = 8;
        // double x,y,z;
        ...
        // std::normal_distribution<double> normdistw(0.0,width/2);
        // std::normal_distribution<double> normdistL(0.0,length/2);
        std::uniform_real_distribution<double> uniformdist(0.0,length);
        std::uniform_real_distribution<double> uniformdistw(0.0,width);
        std::uniform_real_distribution<double> uniformdisttheta(0.0,2*PI);
        ...
        {
            int index = std::distance(vecNode.begin(), it );
            r = uniformdistw(generator);
            theta = uniformdisttheta(generator);

            x[index] = r*cos(theta);
            y[index] = r*sin(theta);
            z[index] = uniformdist(generator);
