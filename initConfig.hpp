...
const double RAD_NUCLEUS        = 5000;               // nm; Haering paper S. pombe
const double DEPTH_NUCLEOLUS    = 1666;                // nm;
...
const int    NUM_BEAD = 250; //1880*2kb = 3.76 Mb  						// 2436 for testing dynamics
...
const double DIFFUSION_CAPTURE_ON   = false;
...
const double P_DISSOCIATE_CONDENSIN = 1E-4;    // a probability per simulation step that an existing interaction is lost (eventually force dependent?). 1E-3 for interphase; 1E-4 for condensin

const double P_DISSOCIATE_MITOSIS   = 1E-2; 
// ... binder tension (loop extrusion mechanism)
const double LOOP_EXTRUSION_ON      = true;
...
/* ----------------------08-08-2018-------------------------- */
const string BINDER_POSITION          =   "uniform"; //"uniform"; // "random1"; "uniform"; "CHIP-seq";
